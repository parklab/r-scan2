...
check.chunked <- function(object, message) {
    if (is.null(object@gatk.regions))
        warning('slot @gatk.regions not found.  This SCAN2 object was probably generated by an older (incompatible) SCAN2 package.')
    if (is.null(object@region))
        warning('slot @region is NULL.  Probably safe to ignore this message.')

    not.covered <- GenomicRanges::countOverlaps(object@gatk.regions, object@region, type='within') == 0
    if (any(not.covered))
